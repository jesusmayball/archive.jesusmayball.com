...
if (isset($_GET["mode"]) && $_GET["mode"] == "alumni") {
...
/*
if (count($db->getAllTicketTypes(true)) !== 0) {
 // && $_GET["override"] !== "996f4d32c3ddb56446ce3a478bda9795") {
...
function submitWaitingList()
{
	...
	try {
		$waitingListID = $db->insertWaitingList($waitingList);
		$waitingList->waiting_list_id = $waitingListID;
		if (!$waitingListID) {
			Layout::htmlExit("Waiting List", "Error inserting blank waiting list entry.");
		}
	} catch (DatabaseException $e) {
		Layout::htmlExit("Waiting List", "A waiting list entry change has already been submitted for $title $firstName $lastName");
	}

	$waitingListVariables = array(
		"\$waitingList->name" => $waitingList->first_name . " " . $waitingList->last_name,
		...
		<form id="waitinglist" class="well form-horizontal" method="post">
			<fieldset>
				...
				<div class="form-group" id="form-group-phone">
					...
					<div class="col-sm-9">
						<div class="input-append">
							<input class="form-control" type="text" name="phone" /><span class="help-inline" style="display: none">Please enter your phone number</span>
						</div>
					</div>
				</div>
				<div class="form-group" id="form-group-desired-ticket">
					...
					<div class="col-sm-9">
						<select id="no_tickets_field" name="desired_ticket_type" class="form-control">
							<?php
							$ticketTypes = removeAlumniTickets($db->getAllTicketTypes(false, true, false));
