...
$bursary_only_for_college = true; //whether they're available to other colleges
//TODO jakub - maybe put this in global config? requires making them properly though...
//bursary tickets are only available for main applicant regardless
...
// echo '<pre>';
// print_r(removeAlumniTickets(Database::getInstance()->getAllTicketTypes(true)));
// echo '</pre>';
...
try {
	...
	$_SESSION["cookiename"] = $auth->cookie_name();
...
if ($_GET["mode"] == "agent") {
	...
	if (userHasPermission($auth->principal(), "reservation")) {
		$mode = "agent";
		$nav = "mode=agent&amp;";
		$navc = $nav;
		$max = 49;
	} else {
		$html = "Authentication failed. You are not allowed to access this page. Please <a href=\"logout.php\" target=\"blank\">log out</a> from Raven and refresh this page.";
		Layout::htmlExit($pageTitle, $html);
	}
} else {
	if (time() >= $config['dates']['college_online_sales'] || $_GET['hacking'] == 'true') {
		//TODO Read in CRSIDs from file
		if (isset($_GET['hacking']) && $_GET['hacking'] == 'true') {
			if (!$auth->principal()) Layout::htmlExit($pageTitle, "Raven failed.");
			$mode = "uni";
			$nav = "hacking=true&amp;";
			$navc = $nav;
			$max = $config["max_guests"];
			$collegePurchase = true;
		} else if (userHasPermission($auth->principal(), "college")) {
			$mode = "uni";
			$nav = "";
			$navc = $nav;
			$max = $config["max_guests"];
			$collegePurchase = true;
		} else if (time() >= $config['dates']['general_sale']) {
			$mode = "uni";
			$nav = "";
			$navc = $nav;
			$max = $config["max_guests"];
			...
			Layout::htmlExit($pageTitle, "Tickets are not yet available online to non-" . $config['college_student_name'] . ". They will be released at " . date('H.i \o\n jS F Y', $config['dates']['general_sale']) . ".");
		}
	} else {
		Layout::htmlExit($pageTitle, "Tickets are not yet available online. They will be released to " . $config['college_student_name'] . " at " . date('H.i \o\n jS F Y', $config['dates']['college_online_sales']) . ". They will be released to all other members of the university at " . date('H.i \o\n jS F Y', $config['dates']['general_sale']) . ".");
	}
}
...
if ($_GET["step"] == "delete_cookie" || $_GET['reset'] == "all") {
	//delete session cookie before headers are sent
	unset($_COOKIE['TICKETING_SESSION']);
	setcookie('TICKETING_SESSION', null, -1, '/');
}

if ($_SESSION["complete"] || $_GET["cancel"] || $_GET['reset'] == "all") {
...
if ($mode != "agent") {
	if (count(removeAlumniTickets(Database::getInstance()->getAllTicketTypes(true))) <= 0) {
		if ($mode == "alumni") {
			Layout::htmlExit($pageTitle, "Sorry, all tickets have sold out. <a href=\"waitinglist.php?mode=alumni&r=" . $config["alumni_key"] . "\">Please join the waiting list</a>");
		} else {
			Layout::htmlExit($pageTitle, "Sorry, all tickets have sold out. <a href=\"waitinglist.php\">Please join the waiting list</a>");
		}
	}
}
...
if ((isset($_SESSION["mode"])) && (!isset($_GET["override"]))) {
	if (($_SESSION["mode"] == "agent") && ($mode != "agent")) {
		header('Location: reserve.php?mode=agent');
	}
	if (($_SESSION["mode"] == "alumni") && ($mode != "alumni")) {
		header('Location: reserve.php?mode=alumni');
	}
...
if ($_GET["step"] == 1 || !isset($_GET["step"])) {
	...
	$_SESSION["auth"] = md5('asd7z$$&' . strval(time()));
	...
	Layout::htmlTop("Ticket Purchase - Personal Details");
?>
	...
	<div class="container purchase-container">
		...
		<form id="step1" class="well form-horizontal" method="post" action="reserve.php?<?php echo $nav ?>step=2">
			<fieldset>
				<p>
					No guest tickets are available to purchase. If you already have a ticket you are not allowed to buy a second - if you do so your second ticket will be voided and you wont be refunded.
				</p>
				...
				<div class="form-group" id="form-group-last-name">
					...
					<div class="col-sm-9">
						<input type="text" id="last_name_field" name="last_name" value="" class="form-control" placeholder="Last Name" /> <span class="help-inline" style="display: none">Please enter your last name</span>
					</div>
				</div>
				...
				<div class="form-group" id="form-group-diet">
					...
					<div class="col-sm-9">
						<select id="diet_field" name="diet" class="form-control">
							<?php Layout::dietOptions(); ?>
						</select> <span class="help-inline" style="display: none">Please
							select dietary requirements</span>
					</div>
				</div>
				...
				if ($mode == "alumni") { ?>
					<div class="form-group" id="form-group-email">
						<label class="control-label col-sm-3">Cambridge Email Address*:</label>
						<div class="col-sm-9">
				...
				<div class="form-group">
					<div class="col-sm-9 col-sm-offset-3">
						...
						<a class="btn btn-danger" href="reserve.php?<?php echo $navc ?>cancel=1">Cancel <span class="glyphicon glyphicon-remove"></span></a>
					</div>
				</div>
			</fieldset>
		</form>
...
//TODO
//I think this is old code and is redundant - pls remove if time is had
// step2 is just a charity guilt trip and the actual guest tickets have been moved to step3
if ($_GET["step"] == 2) {
	if ($_SESSION["fail"])
	...
	if ($_POST["title"]) {
		...
		$_SESSION["principal"]["diet"] = stripslashes(preg_replace('/[^A-Za-z\'\-\.\s+]/', "", $_POST["diet"]));
	...
	if ($errors !== "") {
		...
		$errors .= "<a class='btn btn-primary' href='reserve.php?{$nav}step=1'><span class='icon-white icon-chevron-left'></span> Go Back</a>";
	...
	if ($mode != "agent") {
		...
		if ($bursary_only_for_college && !$collegePurchase) {
			$ticketTypes = removeBursaryTickets($ticketTypes);
		}
	} else {
		$ticketTypes = removeAlumniTickets($db->getAllTicketTypes(true, false, true));
	}
	$ticket_options = "";
	...
	$ticket_options_guest = "";
	foreach ($ticketTypes as $availableTicket) {
		if (!in_array($availableTicket->ticket_type_id, $alumni_categories) && !in_array($availableTicket->ticket_type_id, $bursary_categories)) {
			$ticket_options_guest .= "<option value=\"" . $availableTicket->ticket_type_id . "\"";
			if ((isset($_SESSION["principal"]["ticket_type_id"]))
				&& ($_SESSION["principal"]["ticket_type_id"] == $availableTicket->ticket_type_id)
			) {
				$ticket_options_guest .= " selected ";
			}
			$ticket_options_guest .= ">" . $availableTicket->ticket_type;
			$ticket_options_guest .= " - &#163;" . $availableTicket->price . "</option>";
		}
	}
...
?>
	<script type="text/javascript">
		...
		var maxguests = <?php echo $config["max_guests"] ?>;
		...
		function addTicket(i) {
			...
			if (numberOfDivsPresent < max) {
				...
				<?php if ($mode == "agent") { ?>
					...
					if (numberOfDivsPresent >= maxguests) {
						quantitychecked = confirm("Ticket holders are allowed a maximum of " + maxguests + " guests. Any extra guests must be at the discretion of the presidents. Are you sure you wish to proceed and add this ticket?");
					}
					if ((numberOfDivsPresent < maxguests) || quantitychecked) {
					<?php } ?>
					...
					div.append('<select name="ticket_' + addTicket.count + '_ticket_type_id" id="ticket_' + addTicket.count + '_ticket_type_id_field" class="form-control"><?php echo addslashes($ticket_options_guest) ?></select>');
					...
					var charity_options = '<label class="radio-inline">' +
						...
						'<input name="ticket_' + addTicket.count + '_charity" id="' + inputName + '_field_no" value="0" type="radio">' +
						'<?php echo $charity_no_phrase; ?></label>' +
						'<label class="radio-inline" style="margin-left:0px;">' +
						'<input name="ticket_' + addTicket.count + '_charity" id="' + inputName + '_field_other" value="other" type="radio">' +
				...
				alert("You may only order " + max + " additional tickets");
			}
		}
		...
		$("document").ready(function() {
			//TODO Here
			//TODO jakub - idk why this stuff is here twice, will figure out if I have time
	...
	<div class="container purchase-container">
		...
		<ol class="breadcrumb">
			<li><a href="reserve.php?<?php echo $nav ?>step=1">Personal Details</a></li>
			<li class="active">Charities</li>
			<li>Tickets</li>
			<li>Confirm</li>
			<li>Completion</li>
		</ol>
		<form id="step2" class="well form-horizontal" action="reserve.php?<?php echo $nav ?>step=3" method="post">
			<fieldset>
				<legend>Charities</legend>
				<?php
				...
				<div class="form-group">
					<div class="col-sm-12">
						<a onclick="window.location.href='reserve.php?<?php echo $nav ?>step=1'" class="btn btn-success"><span class="glyphicon glyphicon-chevron-left"></span>
							Back</a>
...
?>
		...
		function addTicket(i) {
			...
			if (numberOfDivsPresent < max) {
				...
				<?php if ($mode == "agent") { ?>
					...
					$("#ticket_" + addTicket.count).append('<div class="form-group" id="form-group-diet"></div>');
					$("#ticket_" + addTicket.count + " #form-group-diet").append('<label class="control-label col-sm-3">Dietary Requirements*:</label>');
					div = $('<div class="col-sm-9"></div>').appendTo("#ticket_" + addTicket.count + " #form-group-diet");
					div.append('<select id="ticket_' + addTicket.count + '_diet_field" name="ticket_' + addTicket.count + '_diet" class="form-control"><?php Layout::dietOptions(); ?></select>');
					div.append('<span class="help-inline" style="display:none">Please select diet</span>');
	...
	<div class="container purchase-container">
		...
		<ol class="breadcrumb">
			<li><a href="reserve.php?<?php echo $nav ?>step=1">Personal Details</a></li>
			<li><a href="reserve.php?<?php echo $nav ?>step=2">Charities</a></li>
			<li class="active">Tickets</li>
			<li>Confirm</li>
			<li>Completion</li>
		</ol>
		<form id="step3" class="well form-horizontal" action="reserve.php?<?php echo $nav ?>step=4" method="post">
			<fieldset>
				<legend>Tickets</legend>
				<div class="form-group">
					<label class="control-label col-sm-3">Name:</label>
					<div class="col-sm-9 form-control-static">
						<?php echo stripslashes($_SESSION["principal"]["title"] . " " . $_SESSION["principal"]["first_name"] . " " . $_SESSION["principal"]["last_name"]) ?>
					</div>
				</div>
				...
				<div class="form-group">
					<div class="col-sm-9 col-sm-offset-3">
						<br>
						<p class="text-danger">Guest tickets are not guaranteed. You may ballot for up to 2 guest tickets and will be informed at a later date wether you have or haven't receieved them.</p>
						<a class="btn btn-primary" id="add_ticket">Add Guest Ticket <span class="glyphicon glyphicon-tag"></span></a>
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-9 col-sm-offset-3">
						<a onclick="window.location.href='reserve.php?<?php echo $nav ?>step=2'" class="btn btn-success"><span class="glyphicon glyphicon-chevron-left"></span>
							Back</a> <a onclick="$('form#step3').submit()" class="btn btn-success">Next <span class="glyphicon glyphicon-chevron-right"></span>
						</a> <a class="btn btn-danger" onclick="window.location.href='reserve.php?<?php echo $navc ?>cancel=1'"><span class="glyphicon glyphicon-remove"></span> Cancel</a>
					</div>
				</div>
			</fieldset>
		</form>
...
if ($_GET["step"] == 4) {
	if ($_SESSION["fail"] || ($_POST["auth2"] != $_SESSION["auth"])) {
		Layout::htmlExit($pageTitle, "An error occured", "There was an error with your application. Please try again later.<br/><a class=\"btn btn-primary\" href=\"reserve.php?step=1\">Back to start</a>");
	}
	...
	if (!empty($_POST)) {
		...
		$errors = "";
		if (count($_SESSION["guests"]) > $max) {
			$errors .= "<div>You have reserved too many guest tickets. You may reserve no more than $max.</div>";
		} else {
			foreach ($_SESSION["guests"] as $key => $value) {
				if ($mode != "alumni") {
		...
		if ($errors !== "") {
			...
			$errors .= "<a class='btn btn-primary' href='reserve.php?{$nav}step=3'><span class='icon-white icon-chevron-left'></span> Go Back</a>";
			Layout::htmlExit("An error occured", $errors);
		}
	}
...
?>
	...
	<script type="text/javascript">
		...
		function toggleSubmit() {
			if ($("#tac").is(":checked") && grecaptcha.getResponse()) {
		...
		var onloadCallback = function() {
			widgetId1 = grecaptcha.render('captcha-challenge', {
				'sitekey': '6LcMw58eAAAAAPyohnMSUj3JYOYHNoZaEIBd-jPh',
				'theme': 'light'
			});
	...
	<div class="container purchase-container">
		...
		<ol class="breadcrumb">
			<li><a href="reserve.php?<?php echo $nav ?>step=1">Personal Details</a></li>
			<li><a href="reserve.php?<?php echo $nav ?>step=2">Charities</a></li>
			<li><a href="reserve.php?<?php echo $nav ?>step=3">Tickets</a></li>
			<li class="active">Confirm</li>
			<li>Completion</li>
		</ol>
		<form id="step4" class="well form-horizontal" action="reserve.php?<?php echo $nav ?>step=5" method="post">
			...
			<div id="main-applicant-tickets">
				...
				<div class="form-group">
					<div class="col-sm-9 col-sm-offset-3">
						<?php
						echo "<a class=\"btn btn-primary\" href=\"reserve.php?{$nav}step=1\"><span class=\"glyphicon-edit glyphicon\"></span> Edit</a><br />";
						?>
					</div>
				</div>
			</div>
			...
			?>
				<div>
					...
					<div class="form-group">
						<div class="col-sm-9 col-sm-offset-3">
							<?php
							echo "<a class=\"btn btn-primary\" href=\"reserve.php?{$nav}step=3\"><span class=\"glyphicon-edit glyphicon\"></span> Edit Additional Tickets</a>";
							?>
						</div>
					</div>
				</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9 form-control-static">
					In order to reduce the May Ball's environmental impact this year, we aim to distribute all tickets electronically via email. As such, we will no longer be providing printed tickets.
				</div>
				<!-- <div class="col-sm-9 col-sm-offset-3">
				<div class="checkbox">
			...
			<div class="form-group">
				<div class="col-sm-9 col-sm-offset-3 form-control-static">By confirming this selection you are committing to buying these tickets. Payment instructions will be shown on the next screen.<br><br><strong>The cost to change the name on a ticket after this point is £40.</strong></div>
			</div>
			<div class="form-group">
				<div class="col-sm-9 col-sm-offset-3">
					<div class="checkbox">
			...
			<div class="form-group">
				<div class="col-sm-9 col-sm-offset-3">
					<div id="captcha-challenge" data-callback="toggleSubmit">
					</div>
				</div>
			</div>
			<div class="form-group">
				<div class="col-sm-9 col-sm-offset-3">
					<a onclick="window.location.href='reserve.php?<?php echo $nav ?>step=3'" class="btn btn-success"><span class="glyphicon glyphicon-chevron-left"></span>
						Back</a> <a onclick="$('form#step4').submit()" id="confirm_button" class="btn btn-success">Confirm <span class="glyphicon glyphicon-chevron-right"></span>
					</a> <a class="btn btn-danger" onclick="window.location.href='reserve.php?<?php echo $navc ?>cancel=1'"><span class="glyphicon glyphicon-remove"></span> Cancel</a>
				</div>
			</div>
		</form>
...
if ($_GET["step"] == 5) {
	...
	if (!isset($_POST["g-recaptcha-response"])) {
		Layout::htmlExit("", "Invalid captcha response, ticket has been voided.<br/><a href=\"../\" class=\"btn btn-primary\">Back to start</a>");
	} else {
		$ip = $_SERVER['REMOTE_ADDR'];
		// post request to server
		$url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($captchaConfigSecretKey) .  '&response=' . urlencode($_POST["g-recaptcha-response"]);
...
?>
	<div class="container purchase-container">
		...
		<ol class="breadcrumb">
			...
			<li><a href="reserve.php?<?php echo $nav ?>step=2">Charities</a></li>
			<li><a href="reserve.php?<?php echo $nav ?>step=3">Tickets</a></li>
			<li><a href="reserve.php?<?php echo $nav ?>step=4">Confirm</a></li>
		...
		if (!isset($_SESSION["app_id"]) && isset($_SESSION["principal"])) {
			...
			$tickets_all = $db->getAllTicketTypes(false, true, true);
			$tickets = removeAlumniTickets($tickets_all);
			$orderDetails .= "<div class=\"form-group\">";
			$orderDetails .= "<label class=\"control-label col-sm-3\">" . $tickets[$_SESSION["principal"]["ticket_type_id"]]->ticket_type . "</label>";
			$orderDetails .= "<div class=\"form-control-static col-sm-9\">&#163;" . sprintf("%01.2f", $tickets[$_SESSION["principal"]["ticket_type_id"]]->price) . "</div>";
			$orderDetails .= "</div>";
			...
			$orderDetails .= "<div class=\"form-control-static col-sm-9\"><strong>&#163;" . sprintf("%01.2f", $order_charity + $tickets[$_SESSION["principal"]["ticket_type_id"]]->price) . "</strong></div>";
			$orderDetails .= "</div>";
			...
			$guestDetails = "<legend>Guest tickets</legend>";
			foreach ($order as $key => $value) {
				$orderDetails .= "<div class=\"form-group\">";
				$orderDetails .= "<label class=\"control-label col-sm-3\">" . $value . " x " . $tickets[$key]->ticket_type . "</label>";
				$orderDetails .= "<div class=\"form-control-static col-sm-9\">&#163;" . sprintf("%01.2f", $value * $tickets[$key]->price) . "</div>";
			...
			foreach ($order as $key => $value) {
				...
				if ($max_key > 0) {
					$key_available = $tickets_all[$key]->maximum - $tickets_all[$key]->ticket_count;
					$key_max_available = $tickets_all[$max_key]->maximum - $tickets_all[$max_key]->ticket_count;
				...
				if ($value > $number_available || ($using_other_max && $value_category > $number_available)) {
					...
					if ($number_available <= 0) {
						//this shouldn't need to happen, but just in case
						//set the ticket type to unavailable, i.e. we have sold out
			...
			if (!$enough_room) {
				...
				$noTicketsLeft = count(removeAlumniTickets($db->getAllTicketTypes(true))) <= 0;
				if (count($soldOut) > 0) {
				...
				$principalTicket->last_name = $_SESSION["principal"]["last_name"];
				...
				$principalTicket->diet = $_SESSION["principal"]["diet"];
				...
				try {
					...
					echo "<p><a class=\"btn btn-primary\" href=\"reserve.php?{$nav}step=1\"><span class=\"glyphicon glyphicon-repeat\"></span> Go back to start</a></p>";
					$db->deleteApp($appID);
				...
				foreach ($_SESSION["guests"] as $key => $value) {
					...
					$guestTicket->diet = $value['diet'];
					$guestTicket->printed_program = $value['printed_program'];
					$guestTicket->is_ballot = 1;
					$guestTicket->valid = 0;
					try {
						...
						echo "<p>Please contact " . $config['ticket_email'] . " if you think this message is in error.</p>";
						echo "<p><a class=\"btn btn-primary\" href=\"reserve.php?{$nav}step=3\">Go back to fix guest tickets</a></p>";
				...
				if (!$result["result"]) {
					...
					$address = $principalTicket->email == null ? $principalTicket->crsid . "@cam.ac.uk" : $principalTicket->email;
				...
				if ($mode == "agent") {
					...
					echo $orderDetails;
					echo "<div class=\"form-group\"><div class=\"col-sm-9 col-sm-offset-3\"><a class=\"btn btn-primary\" href=\"reserve.php?" . $nav . "step=1&amp;cancel=1\">Back to Start <span class=\"glyphicon glyphicon-repeat\"></span></a></div></div>", PHP_EOL;
				} else {
					echo "<p>", PHP_EOL;
					...
					echo "<hr><strong>Important:</strong> You should receive a confirmation email within a couple of minutes, if not immediately. Make sure to check your SPAM folder if you can't find it. If the email hasn't arrived in either you inbox or your SPAM folder, please email <a href='mailto:" . $config['ticket_email'] . "'>" . $config['ticket_email'] . "</a> and Print/Save this page.", PHP_EOL;
					echo "<br><br><button onclick='window.print();'>Print/Save the Page</button><br><br>";
					echo "</p>", PHP_EOL;
					echo $orderDetails, PHP_EOL;
					echo "<p>", PHP_EOL;
					echo "Payment for your primary ticket should be made by bank transfer within 48 hours, as outlined below.", PHP_EOL;
					echo "This information is also included in your confirmation email. Please <b>DO NOT<b> pay for you guest tickets until you receive confirmation you have succesfully won the ballot for them.", PHP_EOL;
					echo "</p>", PHP_EOL;
					echo $guestDetails, PHP_EOL;
					echo "Payment for your guest tickets should not be made until you receieve confirmation you have received them.", PHP_EOL;
					echo "</p>", PHP_EOL;
