...
$jscomp.polyfillIsolated = function (b, e, a, c) {
    var d = b.split("."); b = 1 === d.length; c = d[0]; c = !b && c in $jscomp.polyfills ? $jscomp.polyfills : $jscomp.global; for (var f = 0; f < d.length - 1; f++) { var g = d[f]; if (!(g in c)) return; c = c[g] } d = d[d.length - 1]; a = $jscomp.IS_SYMBOL_NATIVE && "es6" === a ? c[d] : null; e = e(a); null != e && (b ? $jscomp.defineProperty($jscomp.polyfills, d, { configurable: !0, writable: !0, value: e }) : e !== a && (void 0 === $jscomp.propertyToPolyfillSymbol[d] && (a = 1E9 * Math.random() >>> 0, $jscomp.propertyToPolyfillSymbol[d] = $jscomp.IS_SYMBOL_NATIVE ?
        $jscomp.global.Symbol(d) : $jscomp.POLYFILL_PREFIX + a + "$" + d), $jscomp.defineProperty(c, $jscomp.propertyToPolyfillSymbol[d], { configurable: !0, writable: !0, value: e })))
}; $jscomp.underscoreProtoCanBeSet = function () { var b = { a: !0 }, e = {}; try { return e.__proto__ = b, e.a } catch (a) { } return !1 };
...
(function (b, e) { "object" === typeof exports && "undefined" !== typeof module ? module.exports = e() : "function" === typeof define && define.amd ? define(e) : (b = "undefined" !== typeof globalThis ? globalThis : b || self, b.QrScanner = e()) })(this, function () {
    class b {
        constructor(a, c, d, f, g) {
            this._legacyCanvasSize = b.DEFAULT_CANVAS_SIZE; this._preferredCamera = "environment"; this._maxScansPerSecond = 25; this._lastScanTimestamp = -1; this._destroyed = this._flashOn = this._paused = this._active = !1; this.$video = a; this.$canvas = document.createElement("canvas");
            ...
            a.disablePictureInPicture = !0; a.playsInline = !0; a.muted = !0; let k = !1; a.hidden && (a.hidden = !1, k = !0); document.body.contains(a) || (document.body.appendChild(a), k = !0); d = a.parentElement; if (c.highlightScanRegion || c.highlightCodeOutline) {
                f = !!c.overlay; this.$overlay = c.overlay || document.createElement("div"); g = this.$overlay.style; g.position = "absolute"; g.display = "none"; g.pointerEvents = "none"; this.$overlay.classList.add("scan-region-highlight"); if (!f && c.highlightScanRegion) {
                ...
                } c.highlightCodeOutline && (this.$overlay.insertAdjacentHTML("beforeend", '<svg class="code-outline-highlight" preserveAspectRatio="none" style="display:none;width:100%;height:100%;fill:none;stroke:#e9b213;stroke-width:5;stroke-dasharray:25;stroke-linecap:round;stroke-linejoin:round"><polygon/></svg>'),
            ...
            }); a.addEventListener("play", this._onPlay); a.addEventListener("loadedmetadata", this._onLoadedMetaData); document.addEventListener("visibilitychange", this._onVisibilityChange); window.addEventListener("resize", this._updateOverlay); this._qrEnginePromise = b.createQrEngine()
            ...
            !1) {
                return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                    if (!navigator.mediaDevices) return []; let c = () => $jscomp.asyncExecutePromiseGeneratorFunction(function* () { return (yield navigator.mediaDevices.enumerateDevices()).filter(f => "videoinput" === f.kind) }), d; try { a && (yield c()).every(f => !f.label) && (d = yield navigator.mediaDevices.getUserMedia({ audio: !1, video: !0 })) } catch (f) { } try { return (yield c()).map((f, g) => ({ id: f.deviceId, label: f.label || (0 === g ? "Default Camera" : `Camera ${g + 1}`) })) } finally {
                        d && (console.warn("Call listCameras after successfully starting a QR scanner to avoid creating a temporary video stream"),
                            b._stopVideoStream(d))
                    }
                })
        } hasFlash() { const a = this; return $jscomp.asyncExecutePromiseGeneratorFunction(function* () { let c; try { if (a.$video.srcObject) { if (!(a.$video.srcObject instanceof MediaStream)) return !1; c = a.$video.srcObject } else c = (yield a._getCameraStream()).stream; return "torch" in c.getVideoTracks()[0].getSettings() } catch (d) { return !1 } finally { c && c !== a.$video.srcObject && (console.warn("Call hasFlash after successfully starting the scanner to avoid creating a temporary video stream"), b._stopVideoStream(c)) } }) } isFlashOn() { return this._flashOn } toggleFlash() {
            const a =
        ...
        } destroy() { this.$video.removeEventListener("loadedmetadata", this._onLoadedMetaData); this.$video.removeEventListener("play", this._onPlay); document.removeEventListener("visibilitychange", this._onVisibilityChange); window.removeEventListener("resize", this._updateOverlay); this._destroyed = !0; this._flashOn = !1; this.stop(); b._postWorkerMessage(this._qrEnginePromise, "close") } start() {
            const a = this; return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
            ...
            return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                let l, p = !1; c && ("scanRegion" in c || "qrEngine" in c || "canvas" in c || "disallowCanvasResizing" in c || "alsoTryWithoutScanRegion" in c || "returnDetailedScanResult" in c) ? (l = c.scanRegion, d = c.qrEngine, f = c.canvas, g = c.disallowCanvasResizing || !1, k = c.alsoTryWithoutScanRegion || !1, p = !0) : c || d || f || g || k ? console.warn("You're using a deprecated api for scanImage which will be removed in the future.") :
                    console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true."); let r = !!d; try {
                        let m, q;[d, m] = yield Promise.all([d || b.createQrEngine(), b._loadImage(a)]);[f, q] = b._drawToCanvas(m, l, f, g); let n; if (d instanceof Worker) {
                            let h = d; r || b._postWorkerMessageSync(h, "inversionMode", "both"); n = yield new Promise((t, z) => {
                                let x, v, w, y = -1; v = u => {
                                    u.data.id === y && (h.removeEventListener("message", v), h.removeEventListener("error",
                                        w), clearTimeout(x), null !== u.data.data ? t({ data: u.data.data, cornerPoints: b._convertPoints(u.data.cornerPoints, l) }) : z(b.NO_QR_CODE_FOUND))
                            ...
                            t) => window.setTimeout(() => t("Scanner error: timeout"), 1E4)), (() => $jscomp.asyncExecutePromiseGeneratorFunction(function* () { try { var [h] = yield d.detect(f); if (!h) throw b.NO_QR_CODE_FOUND; return { data: h.rawValue, cornerPoints: b._convertPoints(h.cornerPoints, l) } } catch (t) { h = t.message || t; if (/not implemented|service unavailable/.test(h)) return b._disableBarcodeDetector = !0, b.scanImage(a, { scanRegion: l, canvas: f, disallowCanvasResizing: g, alsoTryWithoutScanRegion: k }); throw `Scanner error: ${h}`; } }))()]); return p ? n :
                                n.data
                    } catch (m) { if (!l || !k) throw m; let q = yield b.scanImage(a, { qrEngine: d, canvas: f, disallowCanvasResizing: g }); return p ? q : q.data } finally { r || b._postWorkerMessage(d, "close") }
            })
        } setGrayscaleWeights(a, c, d, f = !0) { b._postWorkerMessage(this._qrEnginePromise, "grayscaleWeights", { red: a, green: c, blue: d, useIntegerApproximation: f }) } setInversionMode(a) { b._postWorkerMessage(this._qrEnginePromise, "inversionMode", a) } static createQrEngine(a) {
            return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                a && console.warn("Specifying a worker path is not required and not supported anymore.");
                return !b._disableBarcodeDetector && "BarcodeDetector" in window && BarcodeDetector.getSupportedFormats && (yield BarcodeDetector.getSupportedFormats()).includes("qr_code") ? new BarcodeDetector({ formats: ["qr_code"] }) : Promise.resolve().then(function () { return e }).then(c => c.createWorker())
            })
            ...
            this._updateOverlay()
        } _onVisibilityChange() { document.hidden ? this.pause() : this._active && this.start() } _calculateScanRegion(a) { let c = Math.round(2 / 3 * Math.min(a.videoWidth, a.videoHeight)); return { x: Math.round((a.videoWidth - c) / 2), y: Math.round((a.videoHeight - c) / 2), width: c, height: c, downScaledWidth: this._legacyCanvasSize, downScaledHeight: this._legacyCanvasSize } } _updateOverlay() {
            requestAnimationFrame(() => {
                if (this.$overlay) {
                    var a = this.$video, c = a.videoWidth, d = a.videoHeight, f = a.offsetWidth, g = a.offsetHeight, k = a.offsetLeft,
                    l = a.offsetTop, p = window.getComputedStyle(a), r = p.objectFit, m = c / d, q = f / g; switch (r) { case "none": var n = c; var h = d; break; case "fill": n = f; h = g; break; default: ("cover" === r ? m > q : m < q) ? (h = g, n = h * m) : (n = f, h = n / m), "scale-down" === r && (n = Math.min(n, c), h = Math.min(h, d)) }var [t, z] = p.objectPosition.split(" ").map((v, w) => { const y = parseFloat(v); return v.endsWith("%") ? (w ? g - h : f - n) * y / 100 : y }); p = this._scanRegion.width || c; q = this._scanRegion.height || d; r = this._scanRegion.x || 0; var x = this._scanRegion.y || 0; m = this.$overlay.style; m.width =
                        `${p / c * n}px`; m.height = `${q / d * h}px`; m.top = `${l + z + x / d * h}px`; d = /scaleX\(-1\)/.test(a.style.transform); m.left = `${k + (d ? f - t - n : t) + (d ? c - r - p : r) / c * n}px`; m.transform = a.style.transform
                }
            })
        } static _convertPoints(a, c) { if (!c) return a; let d = c.x || 0, f = c.y || 0, g = c.width && c.downScaledWidth ? c.width / c.downScaledWidth : 1; c = c.height && c.downScaledHeight ? c.height / c.downScaledHeight : 1; for (let k of a) k.x = k.x * g + d, k.y = k.y * c + f; return a } _scanFrame() {
                        ...
                        if (!(1 >= a.$video.readyState)) {
                            var c = Date.now() - a._lastScanTimestamp, d = 1E3 / a._maxScansPerSecond; c < d && (yield new Promise(g => setTimeout(g, d - c))); a._lastScanTimestamp = Date.now(); try { var f = yield b.scanImage(a.$video, { scanRegion: a._scanRegion, qrEngine: a._qrEnginePromise, canvas: a.$canvas }) } catch (g) { if (!a._active) return; a._onDecodeError(g) } !b._disableBarcodeDetector ||
                                (yield a._qrEnginePromise) instanceof Worker || (a._qrEnginePromise = b.createQrEngine()); f ? (a._onDecode ? a._onDecode(f) : a._legacyOnDecode && a._legacyOnDecode(f.data), a.$codeOutlineHighlight && (clearTimeout(a._codeOutlineHighlightRemovalTimeout), a._codeOutlineHighlightRemovalTimeout = void 0, a.$codeOutlineHighlight.setAttribute("viewBox", `${a._scanRegion.x || 0} ` + `${a._scanRegion.y || 0} ` + `${a._scanRegion.width || a.$video.videoWidth} ` + `${a._scanRegion.height || a.$video.videoHeight}`), a.$codeOutlineHighlight.firstElementChild.setAttribute("points",
                                    f.cornerPoints.map(({ x: g, y: k }) => `${g},${k}`).join(" ")), a.$codeOutlineHighlight.style.display = "")) : a.$codeOutlineHighlight && !a._codeOutlineHighlightRemovalTimeout && (a._codeOutlineHighlightRemovalTimeout = setTimeout(() => a.$codeOutlineHighlight.style.display = "none", 100))
                        } a._scanFrame()
                    })
                })
        } _onDecodeError(a) { a !== b.NO_QR_CODE_FOUND && console.log(a) } _getCameraStream() {
            const a = this; return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                if (!navigator.mediaDevices) throw "Camera not found."; let c = /^(environment|user)$/.test(a._preferredCamera) ?
                    "facingMode" : "deviceId", d = [{ width: { min: 1024 } }, { width: { min: 768 } }, {}], f = d.map(g => Object.assign({}, g, { [c]: { exact: a._preferredCamera } })); for (let g of [...f, ...d]) try { let k = yield navigator.mediaDevices.getUserMedia({ video: g, audio: !1 }), l = a._getFacingMode(k) || (g.facingMode ? a._preferredCamera : "environment" === a._preferredCamera ? "user" : "environment"); return { stream: k, facingMode: l } } catch (k) { } throw "Camera not found.";
            })
        } _restartVideoStream() {
            const a = this; return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                let c =
                    a._paused; (yield a.pause(!0)) && !c && a._active && (yield a.start())
            })
        } static _stopVideoStream(a) { for (let c of a.getTracks()) c.stop(), a.removeTrack(c) } _setVideoMirror(a) { this.$video.style.transform = "scaleX(" + ("user" === a ? -1 : 1) + ")" } _getFacingMode(a) { return (a = a.getVideoTracks()[0]) ? /rear|back|environment/i.test(a.label) ? "environment" : /front|user|face/i.test(a.label) ? "user" : null : null } static _drawToCanvas(a, c, d, f = !1) {
            d = d || document.createElement("canvas"); let g = c && c.x ? c.x : 0, k = c && c.y ? c.y : 0, l = c && c.width ? c.width :
                a.videoWidth || a.width, p = c && c.height ? c.height : a.videoHeight || a.height; f || (f = c && c.downScaledWidth ? c.downScaledWidth : l, c = c && c.downScaledHeight ? c.downScaledHeight : p, d.width !== f && (d.width = f), d.height !== c && (d.height = c)); c = d.getContext("2d", { alpha: !1 }); c.imageSmoothingEnabled = !1; c.drawImage(a, g, k, l, p, 0, 0, d.width, d.height); return [d, c]
        } static _loadImage(a) {
            return $jscomp.asyncExecutePromiseGeneratorFunction(function* () {
                if (a instanceof Image) return yield b._awaitImageLoad(a), a; if (a instanceof HTMLVideoElement ||
            ...
            c, d, f) { if (!(a instanceof Worker)) return -1; let g = b._workerMessageId++; a.postMessage({ id: g, type: c, data: d }, f); return g }
    } b.DEFAULT_CANVAS_SIZE = 400; b.NO_QR_CODE_FOUND = "No QR code found"; b._disableBarcodeDetector = !1; b._workerMessageId = 0; var e = Object.freeze({
