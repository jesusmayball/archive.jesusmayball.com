		...
		$.ajax({
			...
			success: function (json) {
				if (json.error == null) {
					...
					$("#total").text("£" + json.totalCost.toFixed(2));
					$("#outstanding").text("£" + (json.totalCost - json.totalPaid).toFixed(2));
					...
					if (json.totalCost <= json.totalPaid) {
						...
						$('#ticketPaymentsPayAllForm input[name="amount"]').val((json.totalCost - json.totalPaid).toFixed(2));
						$('#ticketPaymentsPayAllForm input[name="submit"]').val("Pay remaining £" + (json.totalCost - json.totalPaid).toFixed(2));
						$('#control-group-ticket-warning').hide();
						$('#ticketPaymentsPaySomeForm textarea[name="notes"]').val("");
