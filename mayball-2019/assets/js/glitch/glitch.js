...
const GLITCH_TRANSITION_DURATION = 200;
...
async function captureScreen() {
    return await new Promise((resolve, reject) => {
        var active = $('.section.active').attr('id');
        if (!screenCache[active]) {
            ...
            screenShotCanvas = screenCache[active].canvas;
            canvasImage = screenCache[active].canvasImage;
            screenCaptured = true;
            resolve();
        }
    });
...
function initialLoader() {
    ...
    var readyStateCheckInterval = setInterval(function() {
        if (document.readyState === "complete" && screenCaptured) {
            // console.log("Delay: " + (Date.now() - startTime));
...
function removePageCanvas() {
    ...
    }, GLITCH_TRANSITION_DURATION - FADE_OUT);
}

async function switchScreens() {
    await captureScreen();
    initialLoader();
}
